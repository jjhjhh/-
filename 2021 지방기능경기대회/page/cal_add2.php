<?php  
	...
	if(isset($_POST['showName'])){
		...
		sql(" INSERT INTO `cal` (`showUid`, `showName`, `showDate`, `showTime`, `organizer`, `place`, `cast`, `rm`, `registDt`, `updtDt`) VALUES (NULL, '$showName', '$showDate', '$showTime', '$organizer', '$place', '$cast', '$rm', NOW(), '') ");
		alert("공연 일정 등록 완료");
		mov("/cal_year.php");
	}
?>
...
<body>
	...
	<div class="wrap2 line s-title view">
		<h2 class="title">공연일정 등록</h2>
		<p>HOME > 행사안내 > 공연 > 공연일정등록</p>
	</div><br>

	<form action="" enctype="multipart/form-data" class="wrap2" method="post">
		공연명 <input type="text" name="showName" required="" placeholder="필수"><br>
		공연일 <input type="date" name="showDate" required="" placeholder="필수"><br>
		공연시간 <input type="text" name="showTime" required="" placeholder="필수(00:00형식)"><br>
		주관 <input type="text" name="organizer"><br>
		공연장소 <input type="text" name="place"><br>
		출연진 <input type="text" name="cast"><br>
		공연내용 <input type="text" name="rm"><br>
